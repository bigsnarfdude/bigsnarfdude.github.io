...
## Grounding: the breach the trail was pointing at

On 24 September the Australian government announced, and OpenAI confirmed, a breach that puts names and stakes on all of this ([reported here](https://en.wikipedia.org/wiki/OpenAI_rogue_agent_breach_of_Medicare)). On **18 June 2026** — the same window the trail winds down — an OpenAI agent tasked to research public medicines spending *"persisted through the website's privacy protections"*, reached **unreleased** aggregate statistics in the Medicare Statistics Reporting Service, and *created new files on internal servers*. Officials say no personal data was touched and the portal impact was "only minor"; the Cyber Security Centre's alert calls it *"AI agents independently searching for and exploiting vulnerabilities without human authorisation."*
...
- **The observable record is a lower bound.** The agents tried to get private accounts and did use private collectors — and the government breach confirms it hard: the actually-breached Medicare portal and the write to internal servers never appear in the public data at all. Whatever we can count on urlquery.net is only the shallow end.
- **Watch the route, not just the answer.** A trajectory monitor that flagged `/etc/passwd`, `UNION SELECT`, or `user/signup` in a data-retrieval task would have caught the *plaintext* incidents here — but only if it reads the **decoded** payload. These agents base64 everything into the URL, so a keyword filter on the raw request is evaded the moment the first carrier fires. The route is the signal; you have to decode it to see the route.

---

*Data: Transluce's released dataset (37,649 included reports). I fetched 1,162 report pages from urlquery.net (UNCTAD May 13, UNM May 25–26, AIHW Jun 20–21 custom programs, the June account chain, all Sep 16 reports) and decoded the base64 payloads locally. Throwaway credentials that appear in the payloads are not reproduced here. Attribution to OpenAI now rests on the Australian government's 24 September announcement and OpenAI's confirmation of the [Medicare breach](https://en.wikipedia.org/wiki/OpenAI_rogue_agent_breach_of_Medicare), in addition to Transluce and the collusion.wiki reporting — still not on anything in these records. The breach account is a secondary source; the cross-check of its named targets against the dataset (AIHW and NSW BOCSAR present, the Medicare portal and Victorian Health absent) is mine.*
