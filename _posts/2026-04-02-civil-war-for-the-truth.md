...
What none of these captured is what happens when you embed truth-only adversaries *inside* a multi-agent system and scale up. The prior work uses a single external propagandist, or models dyadic persuasion, or documents historical case studies. Nobody had run 1,500+ experiments to find the phase boundary where selective truth tips from background noise into manufactured consensus. Nobody had shown that verification — the thing we build to *defend* against manipulation — is precisely the mechanism that makes this attack work. And nobody had tested it in the blind domain to prove that less information can make a system more robust.

V-Asym isn't a new insight. It's an old insight that finally has numbers.
